...
/// <summary>
/// Background service that consumes generation jobs from the channel,
/// runs dotnet build, and retries with error context on failure.
/// Implements state machine: building → success | (retry → generating → building) | failed
/// </summary>
    ...
    ILogger<CompileWorkerService> logger) : BackgroundService
{
    ...
    {
        logger.LogInformation("Processing generation {Id}, state={State}, retries={Retries}",
        ...
        while (job.State == GenerationState.Building && job.RetryCount <= MaxRetries)
        {
            ...
            {
                job.State = GenerationState.Failed;
                job.ErrorMessage = "No output directory set.";
                return;
            }
            ...
            {
                // building → packing (skip packing/uploading in Phase 3 — go straight to success)
                job.State = GenerationStateMachine.Transition(
                    job.State, GenerationEvent.BuildPassed, job);

                // Phase 3: skip packing and uploading, mark as success directly
                job.State = GenerationStateMachine.Transition(
                    job.State, GenerationEvent.ZipCreated, job);
                job.State = GenerationStateMachine.Transition(
                    job.State, GenerationEvent.UploadedToR2, job);

                logger.LogInformation("Generation {Id} completed successfully after {Retries} retries",
                    job.GenerationId, job.RetryCount);
                return;
            }

            // Build failed — extract errors and decide retry vs fail
            ...
            logger.LogWarning("Build failed for {Id} (attempt {Attempt}): {Errors}",
                job.GenerationId, job.RetryCount + 1, errorSummary);

            // Transition: building → generating (retry) or building → failed
            ...
            {
                job.ErrorMessage = $"Build failed after {MaxRetries} retries. Last errors: {errorSummary}";
                logger.LogError("Generation {Id} failed permanently after {MaxRetries} retries",
            ...
            // Overwrite only the fixed files in the output directory
            foreach (var (path, content) in fixedBlocks)
            {
                var fullPath = Path.Combine(job.OutputDirectory, path);
