...
- App-level next-react group (#283) held during the wave. **Correction (same night):** src/StackAlchemist.Web was already on Next 16 (^16.2.12) — the "still on Next 15" claim in the first version of this entry was stale memory. #283 is only 16.2.12 → 16.3.1; its pre-rebase red came from the (since-fixed) template lock desync, and it merges on a clean rebase. The follow-on truth fix: the site's product copy still said "Next.js 15" in 49 places while both shipping templates (V1 via #298, V0-Spark via #268) and the app itself run Next 16 — corrected in fix/nextjs-16-truth-strings.
